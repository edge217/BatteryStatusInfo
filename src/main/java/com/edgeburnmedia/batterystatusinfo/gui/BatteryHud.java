...
public class BatteryHud {
	...
	private static final int PERCENT_COLOUR = Colors.WHITE;
	...
	public void render(BatteryStatus status, DrawContext drawContext) {
        ...
        switch (position) {
			case TOP_LEFT -> {
				drawContext.drawTexture(RenderPipelines.GUI_TEXTURED, texture, 1, 0, 0, 0, SCALE, SCALE, SCALE, SCALE);
				drawContext.drawTextWithShadow(client.textRenderer, text, 23, 7, PERCENT_COLOUR);
			}
			case TOP_RIGHT -> {
				drawContext.drawTexture(RenderPipelines.GUI_TEXTURED, texture, windowWidth - SCALE - 1, 0, 0, 0, SCALE, SCALE, SCALE, SCALE);
				drawContext.drawTextWithShadow(client.textRenderer, text, windowWidth - textWidth - 23, 7, PERCENT_COLOUR);
			}
			case BOTTOM_LEFT -> {
				drawContext.drawTexture(RenderPipelines.GUI_TEXTURED, texture, 1, windowHeight - SCALE - 1, 0, 0, SCALE, SCALE, SCALE, SCALE);
				drawContext.drawTextWithShadow(client.textRenderer, text, 23, windowHeight - textHeight - 6, PERCENT_COLOUR);
			}
			case BOTTOM_RIGHT -> {
				drawContext.drawTexture(RenderPipelines.GUI_TEXTURED, texture, windowWidth - SCALE - 1, windowHeight - SCALE - 1, 0, 0, SCALE, SCALE, SCALE, SCALE);
